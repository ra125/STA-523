...
We use the actual precinct numbers from NYPD website to subset the actual Manhattan precincts between o to 34. We got the list as 1,5,6,7,9,10,13,14,17,18,19,20,22,23,24,25,26,28,30,32,33,34. We made separate convex hulls for each precinct (restoring the precinct number for later use) using the function 'gConvexHull' from 'rgeos' library. We then merged all the convex hulls and made a single geojson file called 'precinct.json' (using the function 'writeOGR') as below:
...
NOTE: The boundaries are highly overlapping since we have a number of outliers for each precinct. We see from the Manhattan plot above in section 1 that we have the concentration of points correct but the boundaries are being spoilt just due outliers. We have planned to get rid of them by the following methods (which we will implement in the coming week):

1) Sort all lat/long pairs numerically. Select only a 95% quantile. That way we can get rid of a few outliers hopefully

2) Check say 5 nearest neighbors of each point and if they all lie very far away then remove them but that is very complex

3) Remove 80% of the observations randomly (assuming 80% removes most outliers) and just use the left out 20% to make boundary and do this a 1000 times and take the intersection of all the 1000 polygons.
