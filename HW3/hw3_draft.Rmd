...
check_packages(c("data.table","rgeos","ggmap","rgdal","maptools", "dplyr","stringr","lubridate","leafletR"))
...
### Just to plot what the boundaries look like..but it'll take some time...so skip it 
# plot(object,col="red")
# plot(ch[[2]],add=TRUE, col="black") 
#Boundary of precinct 1. It is bad due to outliers!! We will get rid of them next week.


#### Into GeoJson

#True precinct numbers off of the NYPD website. 
true_p=c(1,5,6,7,9,10,13,14,17,18,19,20,22,23,24,25,26,28,30,32,33,34)
true_p=true_p+1
#require package 'leafletR'
for(i in 1:length(true_p) )
{
d=true_p[i]
chch=ch[[d]]
filename <- paste0("boundary",d-1)
coords<-as.data.frame(chch@polygons[[1]]@Polygons[[1]]@coords)
toGeoJSON(coords, name=filename)
}
