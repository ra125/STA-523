...
Here, we are collecting the location data for Denny's. Since the location service provider for Denny's - Where2GetIt- has a fairly high limit for radius, which is 3000 miles, we base the focal points for location collection in the following five states: Utah, DC, Kansas, Hawaii, and Alaska. We pick Utah to collect location points on the West Coast and DC for the East Coast. And then to make sure that no location information is lost especially regarding the branches in the middle east, we also collect the data from the 1000 miles radius area from Kansas. Finally, we collect the data from Alaska and Hawaii since they are far from the mainland.  

First, using the for loop function, we download the html pages from the location provider and save them under dennys folder. So the five data files contain the information of the dennys branches within the 3000 miles radius of the centroids of the five states mentioned above. Since the limit of each query set by Wehre2GetIt is 1000, each datasheet contains the location information of 1000 branches within the set radius from the focal points we set.  

Second, we use the loop function again to parse the data from the html pages downloaded under dennys folder. After calling each of the raw dat sheet saved in the first step above, we obtain list objects that contain various information of each branch, and then extract pertimant information for this task from those individual lists: name (Dennys), unique ID, address1, city, state, country, zip code, and phone number. After saving them as individual data frame, we bind them all, which then is 5000 observations, and delete duplicated ones. Finally, we exlude the branches that are outside of the US. The final dataset to be used for Task 3 and other additional visualization tasks is called "dennys_data" that is both defined in Environment and saved as an Rdata file under dennys folder. The dennys dataframe contains 1601 observations and 10 variables. 
...
Function 'rdist.earth()' from package 'fields' is used to calculate distance on earth. Firstly, this function takes latitude and longitude values in degrees and not radians (we tested this out). Also, we return the distance in kms. Secondly, it assumes the earth’s radius to be 6378.388 km. According to Wikipedia this number seems to be the equatorial radius (the maximum radius). Because earth is not a perfect sphere, however, the radius declines as one moves to the poles reaching a polar minimum of about 6,357 km. The mean radius is 6371 km and thus we use R=6371 in our command. (inspiration for this claim: http://www.r-bloggers.com/great-circle-distance-calculations-in-r/)
