...
if(as.character( length(unique(names(input)))==length(input)) =="TRUE"){  #begin second ifelse
 ...
 # 2: Testing if the object has both weights and edges as lists
...
if(as.character( sum(grep("weights",names(input[[1]])))>0 & sum(grep("edges",names(input[[1]])))>0 )=="TRUE"){
 ...
 # 3: Testing if the types of edges are integer and weights are double
t_obj4_1 =data.frame(table(is.na(input[[1]]$edges) ) )
t_obj4_2 =t_obj4_1[]
...
In case you are trying each object, use the codes below. these are from the test script created by the prof.
```{r}
#good ones
