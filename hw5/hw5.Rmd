...
options(warn=-1)
x=xtable(score_slice_data, type="html")
options(warn=0)
```
...
The code to generate the following table is included in the slice.R file, which is saved under hw5 directory. The time shown in the total time column are the times for the whole function. It doesnt display the time/iteration right now but we will change that ASAP.
```{r}
